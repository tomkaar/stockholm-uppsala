...
export const Station = ({ station, isLast, isLoading }: IStation) => {
  ...
  return (
    <div className={`relative flex flex-row gap-4 ${isLoading ? "animate-pulse" : ""}`}>
      <div className={`mt-[6px] w-2 h-2 rounded-full ${isLoading ? "bg-slate-200 dark:bg-slate-700" : "bg-slate-700"}`} />
      {!isLast && (
        <div className={`absolute top-6 left-[3px] w-[1px] h-[75%] ${isLoading ? "bg-slate-200 dark:bg-slate-700" : "bg-slate-700"}`} />
      )}
        ...
        <div className="flex flex-row justify-between">
          <div>
            {isLoading ? (
              ...
              <h4 className="text-md font-medium leading-4 text-slate-900 dark:text-slate-50">
                {station?.namn ?? ""}
              </h4>
            )}
