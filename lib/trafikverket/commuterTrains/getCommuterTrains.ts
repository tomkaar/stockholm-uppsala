...
export const getCommuterTrains = async (
  fromStation: string,
  toStation: string,
  selectedDay: string
) => {
  ...
  const allPendeltag = await fetchFromTrafikverket(
    getAdvertisedTrainIdentQuery(toStation)
  )
    .then((res) => res.json())
    .then(
      (res) => res.RESPONSE.RESULT[0].TrainAnnouncement[0].AdvertisedTrainIdent
    );
