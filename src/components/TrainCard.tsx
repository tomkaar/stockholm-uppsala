...
export const TrainCard = ({ announcement }: TTrainCard) => {
  const [displayAdditionalInformation, set_displayAdditionalInformation] = useState(false)

  const isMalartag = announcement?.ProductInformation?.find(info => info.Code === "PNA014")
  const isSJRegional = announcement?.ProductInformation?.find(info => info.Code === "PNA025")
  ...
  return (
    <div className={`bg-slate-50 dark:bg-slate-800 ${isCanceled || hasDepartured ? 'opacity-50' : ''} rounded-lg`}>
      <button className="text-left w-full p-2 flex flex-row justify-between" onClick={() => set_displayAdditionalInformation(prev => !prev)}>
        <div className="flex flex-row items-center gap-2">
          <div className="rounded-full bg-slate-200/50 dark:bg-slate-700/50 w-10 h-10 mr-1 relative">
            <span className="absolute top-[50%] left-[50%] translate-x-[-50%] translate-y-[-50%]">{isMalartag ? "🚈" : isSJRegional ? "🚂" : "🚂"}</span>
          </div>
          ...
          <div className="flex flex-col">
            <strong className="text-base text-slate-900 font-semibold dark:text-slate-300">
              ...
              <span className="text-slate-700 dark:text-slate-400 font-normal"> {announcement.AdvertisedTrainIdent}</span>
            </strong>

            <Time announcement={announcement} />

            {hasDeviations && (
              <ul className="text-sm text-orage-600 dark:text-orange-600">
...
const Time = ({ announcement }: TTime) => {
  ...
  if (isDelayed) {
    return (
      <span className="text-sm text-slate-500 dark:text-slate-400">
        {hasDepartured && (
          <span className="mr-1">Lämnat stationen:</span>
        )}
        ...
        <time className="text-rose-500 dark:text-rose-800 ml-2 font-bold" dateTime={dayjs(announcement.EstimatedTimeAtLocation).format("HH:mm")}>
  ...
  return (
    <span className="text-sm text-slate-500 dark:text-slate-400">
      {hasDepartured && (
        <span className="mr-1">Lämnade stationen:</span>
      )}
